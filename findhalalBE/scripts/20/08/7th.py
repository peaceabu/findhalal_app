...
# # Define room 2 boundaries (starting at (4800, 0) for example)
# room2_dimensions = [(4800, 0), (4800 + width2, 0), (4800 + width2, height2), (4800, height2)]
# room2_boundary = patches.Polygon(room2_dimensions, closed=True, linewidth=3, edgecolor='blue', facecolor='none')
# ax.add_patch(room2_boundary)

# Generate grid lines for both rooms
...
# for x in np.arange(4800, 4800 + width2 + cell_size, cell_size):
#     ax.axvline(x=x, color='grey', linestyle='-', linewidth=0.5)
# for y in np.arange(0, height2 + cell_size, cell_size):
#     ax.axhline(y=y, color='green', linestyle='-', linewidth=0.5)

# Overlay finer grid
# for x in np.arange(-2000, width1 + fine_cell_size, fine_cell_size):
#     ax.axvline(x=x, color='blue', linestyle='--', linewidth=0.5)
# for y in np.arange(0, height1 + fine_cell_size, fine_cell_size):
#     ax.axhline(y=y, color='blue', linestyle='--', linewidth=0.5)

# for x in np.arange(0, 4800 + width2 + fine_cell_size, fine_cell_size):
#     ax.axvline(x=x, color='orange', linestyle='--', linewidth=0.5)
# for y in np.arange(0, height2 + fine_cell_size, fine_cell_size):
#     ax.axhline(y=y, color='orange', linestyle='--', linewidth=0.5)
